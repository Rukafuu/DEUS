...
int deus_read_binary(const char *path,DeusProgram *out,char *e,size_t cap){
    ...
    if(get16(b+10)!=0u||get32(b+36)!=0u){free(b);return fail(e,cap,"unsupported header flags");}
    ...
    at=co;while(at<co+cl){uint8_t op=b[at++];if(op<DEUS_OMNI||op>DEUS_HOST_CALL)goto corrupt;uint32_t arg=0;int64_t immediate=0;if(operand_opcode(op)){if(at+4>co+cl)goto corrupt;arg=get32(b+at);at+=4;if((string_operand(op)&&arg>=count)||((op==DEUS_BIND||op==DEUS_LOAD)&&arg>=DEUS_MAX_LOCALS)||(op==DEUS_CONST_BOOL&&arg>1u))goto corrupt;}else if(immediate_opcode(op)){if(at+8>co+cl)goto corrupt;immediate=(int64_t)get64(b+at);at+=8;}DeusInstruction *next=(DeusInstruction*)realloc(out->code,(out->code_count+1u)*sizeof(*next));if(!next)goto oom;out->code=next;out->code[out->code_count++]=(DeusInstruction){op,arg,immediate};}free(b);if(!deus_validate_program(out,e,cap)){deus_program_free(out);return 0;}return 1;
