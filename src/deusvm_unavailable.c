...
else if(in.opcode==DEUS_EMIT||in.opcode==DEUS_DEBUG){FILE *stream=in.opcode==DEUS_EMIT?out:stderr;Value v=st[--sp];if(v.kind==V_TEXT||v.kind==V_STRING)fwrite(v.data,1,v.len,stream);else if(v.kind==V_NULL)fputs("null",stream);else if(v.kind==V_BOOL)fputs(v.scalar?"true":"false",stream);else if(v.kind==V_I64)fprintf(stream,"%lld",(long long)v.scalar);else if(v.kind==V_MANAGED&&!deus_value_write_json(&v.managed,stream)){drop(&v);rc=fail(in.opcode==DEUS_EMIT?"EMIT failed to serialize compound value":"DEBUG failed to serialize compound value");break;}drop(&v);}
